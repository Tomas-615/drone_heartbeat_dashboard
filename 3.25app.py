...
# 通信日志条目
class CommLogEntry:
    def __init__(self, timestamp: str, source: str, target: str, message: str, msg_type: str = "INFO"):
...
def update_comm_system():
    """更新通信系统状态"""
    now = datetime.now(beijing_tz).strftime("%H:%M:%S")
    
    if st.session_state.drone_status == "在线":
        st.session_state.gcs_status = NodeStatus.ONLINE
        st.session_state.obc_status = NodeStatus.ONLINE
        st.session_state.fcu_status = NodeStatus.ONLINE
    elif st.session_state.drone_status == "掉线":
        st.session_state.gcs_status = NodeStatus.ONLINE
        st.session_state.obc_status = NodeStatus.OFFLINE
        st.session_state.fcu_status = NodeStatus.OFFLINE
    
    st.session_state.link_gcs_obc = st.session_state.gcs_status == NodeStatus.ONLINE and st.session_state.obc_status == NodeStatus.ONLINE
    st.session_state.link_obc_fcu = st.session_state.obc_status == NodeStatus.ONLINE and st.session_state.fcu_status == NodeStatus.ONLINE
    
    if st.session_state.link_gcs_obc and st.session_state.link_obc_fcu:
        st.session_state.link_delay = round(random.uniform(20, 35), 1)
        st.session_state.packet_loss = round(random.uniform(0, 0.5), 1)
    else:
        st.session_state.link_delay = None
        st.session_state.packet_loss = None
    
    # 模拟通信日志
    if len(st.session_state.heartbeat_history) % 5 == 0 and len(st.session_state.heartbeat_history) > 0:
        msg_types = ["HEARTBEAT", "GPS_RAW_INT", "SYS_STATUS", "ATTITUDE", "GLOBAL_POSITION_INT"]
        msg_type = random.choice(msg_types)
        
        log_entry = CommLogEntry(
            timestamp=now,
            source="FCU",
            target="OBC",
            message=f"MAVLink: {msg_type}",
            msg_type="INFO"
        )
        st.session_state.comm_logs.insert(0, log_entry)
        
        log_entry2 = CommLogEntry(
            timestamp=now,
            source="OBC",
            target="GCS",
            message=f"Forward: {msg_type}",
            msg_type="INFO"
        )
        st.session_state.comm_logs.insert(0, log_entry2)
        
        if random.random() < 0.3:
            wp_num = random.randint(1, 10)
            log_entry3 = CommLogEntry(
                timestamp=now,
                source="FCU",
                target="GCS",
                message=f"WP_REACHED #{wp_num}",
                msg_type="ACK"
            )
            st.session_state.comm_logs.insert(0, log_entry3)
        
        while len(st.session_state.comm_logs) > 50:
...
def generate_topology_figure():
    ...
    nodes = {
        "GCS": {"x": 0, "y": 1, "label": "地面站<br>GCS", "ip": "192.168.1.100", "status": st.session_state.gcs_status},
        "OBC": {"x": 0.5, "y": 0, "label": "机载计算机<br>OBC", "ip": "Raspberry Pi4", "status": st.session_state.obc_status},
        "FCU": {"x": 1, "y": 1, "label": "飞控<br>FCU", "ip": "PX4/ArduPilot", "status": st.session_state.fcu_status}
    }
    ...
    gcs_obc_color = "#00FF00" if st.session_state.link_gcs_obc else "#FF0000"
    fig.add_trace(go.Scatter(
        x=[nodes["GCS"]["x"], nodes["OBC"]["x"]],
        y=[nodes["GCS"]["y"], nodes["OBC"]["y"]],
        mode='lines',
        line=dict(color=gcs_obc_color, width=3, dash='solid'),
        name='GCS-OBC链路',
        hoverinfo='text',
        text=f'UDP:14550<br>状态: {"正常" if st.session_state.link_gcs_obc else "异常"}'
    ))
    
    obc_fcu_color = "#00FF00" if st.session_state.link_obc_fcu else "#FF0000"
    fig.add_trace(go.Scatter(
        x=[nodes["OBC"]["x"], nodes["FCU"]["x"]],
        y=[nodes["OBC"]["y"], nodes["FCU"]["y"]],
        mode='lines',
        line=dict(color=obc_fcu_color, width=3, dash='solid'),
        name='OBC-FCU链路',
        hoverinfo='text',
        text=f'MAVLink<br>状态: {"正常" if st.session_state.link_obc_fcu else "异常"}'
    ))
    
    fig.add_annotation(x=0.25, y=0.55, text="▶", showarrow=False, font=dict(size=20, color="#00AAFF"), opacity=0.8)
    fig.add_annotation(x=0.75, y=0.45, text="▶", showarrow=False, font=dict(size=20, color="#00AAFF"), opacity=0.8)
    fig.add_annotation(x=0.25, y=0.45, text="◀", showarrow=False, font=dict(size=20, color="#00AAFF"), opacity=0.8)
    fig.add_annotation(x=0.75, y=0.55, text="◀", showarrow=False, font=dict(size=20, color="#00AAFF"), opacity=0.8)
    
    fig.add_annotation(x=0.35, y=0.65, text="GCS→OBC→FCU", showarrow=False, font=dict(size=10, color="#888888"), bgcolor="rgba(255,255,255,0.7)")
    fig.add_annotation(x=0.65, y=0.35, text="FCU→OBC→GCS", showarrow=False, font=dict(size=10, color="#888888"), bgcolor="rgba(255,255,255,0.7)")
    ...
    for node_id, node_info in nodes.items():
        ...
        status_str = get_status_str(node_info["status"])
        color = status_colors.get(status_str, "#888888")
        
        fig.add_trace(go.Scatter(
            x=[node_info["x"]],
            y=[node_info["y"]],
            mode='markers+text',
            marker=dict(symbol=symbol, size=size, color=color, line=dict(color="white", width=2)),
            text=node_info["label"],
            textposition="bottom center" if node_id == "OBC" else "top center",
            textfont=dict(size=12, color="white"),
            name=node_id,
            hoverinfo='text',
            hovertext=f'{node_info["label"]}<br>IP: {node_info["ip"]}<br>状态: {status_str}'
        ))
    
    fig.update_layout(
        title=dict(text="通信链路拓扑与数据流", font=dict(size=16, color="white"), x=0.5),
        showlegend=False,
        hovermode='closest',
        xaxis=dict(showgrid=False, showticklabels=False, zeroline=False, range=[-0.3, 1.3]),
        yaxis=dict(showgrid=False, showticklabels=False, zeroline=False, range=[-0.3, 1.3]),
        plot_bgcolor='#1a1a2e',
...
def load_obstacles_from_file():
    ...
    try:
        ...
        for zone in loaded_zones:
            ...
            obstacle_zones.append({"coordinates": tuple_coords, "height": zone["height"]})
...
def save_obstacles_to_file():
    try:
        ...
        for zone in st.session_state.obstacle_zones:
            ...
            serializable_zones.append({"coordinates": list_coords, "height": zone["height"]})
    ...
    for drawing in drawings:
        if drawing["geometry"]["type"] == "Polygon":
            ...
            if polygon_coords not in existing_coords:
                new_obstacle = {"coordinates": polygon_coords, "height": 50}
...
if "flight_speed" not in st.session_state:
    st.session_state.flight_speed = 5.0
...
if "obc_status" not in st.session_state:
    st.session_state.obc_status = NodeStatus.CONNECTING
if "fcu_status" not in st.session_state:
    st.session_state.fcu_status = NodeStatus.CONNECTING
if "link_gcs_obc" not in st.session_state:
    st.session_state.link_gcs_obc = True
if "link_obc_fcu" not in st.session_state:
    st.session_state.link_obc_fcu = False
...
# ===================== 导航菜单 =====================
menu = st.sidebar.selectbox("功能页面", ["航线规划", "飞行监控"])

with st.sidebar.expander("坐标系设置"):
    coord = st.radio("输入坐标系", ["WGS-84", "GCJ-02(高德/百度)"],
...
if menu == "航线规划":
    ...
    with col1:
        ...
        m = folium.Map(location=[center_lat, center_lon], zoom_start=18, min_zoom=10, max_zoom=22,
                       tiles="cartodb positron", prefer_canvas=True, zoom_control=True,
                       scrollWheelZoom=True, dragging=True)

        folium.TileLayer(tiles='https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}',
                         attr='卫星图', name='卫星地图', max_zoom=18).add_to(m)
        folium.TileLayer(tiles='https://cartodb-basemaps-{s}.global.ssl.fastly.net/light_all/{z}/{x}/{y}.png',
                         attr='2D平面图', name='2D平面地图', max_zoom=22).add_to(m)
        folium.LayerControl().add_to(m)

        if st.session_state.point_a:
            folium.Marker(location=st.session_state.point_a, popup="起点A", tooltip="A点",
                         icon=folium.Icon(color="red"), draggable=True).add_to(m)
        if st.session_state.point_b:
            folium.Marker(location=st.session_state.point_b, popup="终点B", tooltip="B点",
                         icon=folium.Icon(color="green"), draggable=True).add_to(m)
        
        for i, zone in enumerate(st.session_state.obstacle_zones):
            folium.Polygon(locations=zone["coordinates"], popup=f"障碍物区域 {i+1}<br>高度: {zone.get('height', 50)} 米",
                          color="red", fill_color="red", fill_opacity=0.3, weight=2).add_to(m)
        
        if st.session_state.point_a and st.session_state.point_b:
            folium.PolyLine(locations=[st.session_state.point_a, st.session_state.point_b],
                           color="gray", weight=2, dash_array="5, 5", popup="直线路径").add_to(m)
            ...
            for mode, path in st.session_state.calculated_routes.items():
                if path and len(path) > 1:
                    ...
                    folium.PolyLine(locations=path, color=color, weight=weight, opacity=opacity, popup=f"{mode}路径").add_to(m)

        draw = Draw(draw_options={"polyline": False, "polygon": True, "circle": False, "rectangle": False, "marker": False},
                   edit_options={"edit": False})
        draw.add_to(m)

        map_data = st_folium(m, width=900, height=600, key="map1")
    ...
    with col2:
        ...
        with col_drag_a:
            if st.button("拖拽A点", use_container_width=True, type="primary" if st.session_state.drag_mode == 'drag_a' else "secondary"):
                st.session_state.drag_mode = 'drag_a'
                st.success("拖拽A点模式已激活！")
                st.rerun()
        with col_drag_b:
            if st.button("拖拽B点", use_container_width=True, type="primary" if st.session_state.drag_mode == 'drag_b' else "secondary"):
                st.session_state.drag_mode = 'drag_b'
                st.success("拖拽B点模式已激活！")
                st.rerun()
        ...
        route_mode = st.selectbox("选择航线规划模式", [mode.value for mode in RouteMode], index=2, key="route_mode_select")
            ...
            if not st.session_state.point_a or not st.session_state.point_b:
                ...
                with st.spinner("计算航线中..."):
                    ...
                    for mode in RouteMode:
                        ...
                        path = get_obstacle_avoidance_path(st.session_state.point_a, st.session_state.point_b,
                                                          combined_polygon, avoidance_distance, mode)
            ...
            for mode, path in st.session_state.calculated_routes.items():
                if path and len(path) > 1:
                    ...
                    for i in range(len(path) - 1):
                        total_distance += calculate_distance(path[i][0], path[i][1], path[i+1][0], path[i+1][1])
...
# ===================== 页面2：飞行监控 =====================
    ...
    col_topology, col_stats = st.columns([2, 1])
    
    with col_topology:
        topology_fig = generate_topology_figure()
        st.plotly_chart(topology_fig, use_container_width=True)
        
        st.markdown("""
        <div style="background-color: #2d2d44; padding: 8px 16px; border-radius: 8px; margin-top: 10px;">
            <div style="display: flex; justify-content: center; gap: 30px;">
                <span style="color: #00AAFF;">▶ GCS → OBC → FCU</span>
                <span style="color: #00AAFF;">◀ FCU → OBC → GCS</span>
            </div>
        </div>
    ...
    with col_stats:
        st.markdown("#### 链路状态")
        gcs_obc_status = "正常" if st.session_state.link_gcs_obc else "异常"
        gcs_obc_color = "🟢" if st.session_state.link_gcs_obc else "🔴"
        st.markdown(f"{gcs_obc_color} **GCS ↔ OBC**: {gcs_obc_status}")
        
        obc_fcu_status = "正常" if st.session_state.link_obc_fcu else "异常"
        obc_fcu_color = "🟢" if st.session_state.link_obc_fcu else "🔴"
        st.markdown(f"{obc_fcu_color} **OBC ↔ FCU**: {obc_fcu_status}")
        
        if st.session_state.link_delay is not None:
            st.markdown(f"⏱️ **延迟**: ~{st.session_state.link_delay} ms")
            st.markdown(f"📊 **丢包率**: {st.session_state.packet_loss}%")
        else:
            st.markdown("⏱️ **延迟**: -- ms")
            st.markdown("📊 **丢包率**: --%")
        ...
        st.markdown("---")
        st.markdown("#### 节点信息")
        st.markdown("📡 **GCS**: 192.168.1.100")
        st.markdown("🖥️ **OBC**: Raspberry Pi4")
        st.markdown("🚁 **FCU**: PX4/ArduPilot")
    ...
    with col_filter1:
        log_filter = st.selectbox("日志类型", ["全部", "INFO", "ACK", "WARNING", "ERROR"])
    with col_filter2:
        source_filter = st.selectbox("来源", ["全部", "GCS", "OBC", "FCU"])
    with col_filter3:
        st.markdown("#### &nbsp;")
    ...
    if log_filter != "全部":
        filtered_logs = [log for log in filtered_logs if log.msg_type == log_filter]
    if source_filter != "全部":
    ...
    if filtered_logs:
        for log in filtered_logs[:20]:
            if log.msg_type == "ACK":
                color = "#00FF00"
                icon = "✅"
            elif log.msg_type == "WARNING":
                color = "#FFA500"
                icon = "⚠️"
            elif log.msg_type == "ERROR":
                color = "#FF0000"
                icon = "❌"
            else:
                color = "#00AAFF"
                icon = "📡"
            st.markdown(f'<span style="color: {color};">[{log.timestamp}] {icon} {log.source} → {log.target}: {log.message}</span>',
                       unsafe_allow_html=True)
        if len(st.session_state.comm_logs) > 20:
            st.caption(f"共 {len(st.session_state.comm_logs)} 条日志，仅显示最近20条")
    else:
        st.info("暂无通信日志")
    ...
    with col1:
        ...
        if st.session_state.flight_simulation_active:
            col1.metric("当前航点", f"{current_wp} / {total_wp}", delta="飞行中")
        else:
            col1.metric("当前航点", f"{current_wp} / {total_wp}", delta="就绪")
    ...
    with col8:
        ...
        if st.session_state.flight_simulation_active:
            col8.metric("模拟电量", f"{battery:.1f}%", delta="- 放电中")
        else:
    ...
    if st.session_state.flight_path and len(st.session_state.flight_path) > 1:
        ...
        m2 = folium.Map(location=[center_lat, center_lon], zoom_start=18, tiles="cartodb positron")
        
        folium.PolyLine(locations=st.session_state.flight_path, color="blue", weight=3, opacity=0.6, popup="规划飞行路径").add_to(m2)
        ...
        for i, waypoint in enumerate(st.session_state.flight_path):
            ...
            folium.CircleMarker(location=waypoint, radius=6 if i in [0, len(st.session_state.flight_path)-1] else 4,
                               color=color, fill=True, fill_color=color,
                               popup=f"航点 {i+1}" if i not in [0, len(st.session_state.flight_path)-1] else ("起点" if i==0 else "终点")).add_to(m2)
        
        if st.session_state.current_position:
            folium.Marker(location=st.session_state.current_position, popup=f"无人机实时位置 (航点 {st.session_state.current_waypoint_index+1})",
                         tooltip="✈️ 无人机", icon=folium.Icon(color="red", icon="plane", prefix="fa")).add_to(m2)
            ...
            if st.session_state.current_waypoint_index + 1 < len(st.session_state.flight_path):
                ...
                folium.PolyLine(locations=[st.session_state.current_position, next_wp], color="green", weight=3,
                               dash_array="10, 10", popup="当前飞行航段").add_to(m2)

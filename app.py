...
def check_offline(history):
    ...
    time_diff = (datetime.now() - latest_entry['timestamp']).total_seconds()
...
def check_offline(history):
    ...
    **关键参数：**
    ...
    - 当前系统时间: `{datetime.now().strftime('%H:%M:%S')}`
    - 最后心跳时间: `{st.session_state.last_heartbeat_time.strftime('%H:%M:%S') if st.session_state.heartbeat_history else 'N/A'}`
    - 时间差: `{(datetime.now() - st.session_state.last_heartbeat_time).total_seconds() if st.session_state.heartbeat_history else 0:.2f}` 秒
    """)
...
if __name__ == "__main__":
    # 如果在本地运行，可以预先添加一些模拟数据以便展示
    if not st.session_state.heartbeat_history:
        for _ in range(10):
            # 生成一些历史数据，时间设置为过去的时刻
            fake_time = datetime.now() - timedelta(seconds=random.uniform(0, 20))
            fake_data = {
                "timestamp": fake_time,
                "drone_id": DRONE_ID,
                "status_code": 200,
